...
class plateforme(pygame.sprite.Sprite): 
    def __init__(self, x=1, y=SOL_HAUT, haut=SOL_HAUT, long=SOL_LONG, image=SOL_IMG, ) :
        ...
        pygame.sprite.Sprite.__init__(self, sol) #Ajoute notre plateforme au groupe "sol". sol.sprites() est la liste des plateformes
        
    def move(self):
        self.rect.move_ip(-SPEED,0)   #Défile à la vitesse SPEED (positive, donc on met un - pour aller à gauche)
        if (self.rect.right < 0):     #si on est sorti de l'écran
            self.kill()               #on le supprime
        if (SCREEN_WIDTH - self.long < self.rect.right < SCREEN_WIDTH) : #si le dernier ne couvre plus tout sur la droite, il faut ajouter un nouveau
            plateforme(self.rect.right)    #On en met un nouveau juste après

#initialisation du sol
longueur_totale = 0
while longueur_totale < SCREEN_WIDTH :   #on rajoute des bouts de sol, on additionne leur longueur et quand on a couvert tout l'écran on s'arrête.
